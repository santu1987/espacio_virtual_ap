...
class paginacion 
{
    ...
    public $nom_fun;
    public $parametro;
	function __construct($actual,$num_rows,$nom_fun,$parametro)
	{
		$this->actual=$actual;
...
////////////////////////////////////////////////////	
	function crear_paginacion()
	{
       ...
       //valido que si es la 1era no me monte links si es >1 calcula a cual debe ir si pulsa el link determinado...
        if($this->actual > 0)
        {
        	$primera=' <li><a href="#" " onclick="'.$this->nom_fun.'('.$this->parametro.'0,'.$this->limit.',0)">«</a></li>';
        }
        else
        {
        ...
        {
            $ultima.='<li><a href="#" class="button round" onclick="'.$this->nom_fun.'('.$this->parametro.','.((($this->cuantos_enlaces)-1)*$this->limit).','.$this->limit.','.(($this->cuantos_enlaces)-1).')">»</a></li>';
        }
        else
        {
        ...
        for($i=$this->primera_pag; $i<=$this->ultima_pag-1; $i++) 
        {
            ...
            if($i == $this->actual)
            {
                $cuerpo_pag.='<li class="active" onclick="'.$this->nom_fun.'('.$this->parametro.','.$i.','.$this->limit.','.$i.')"><a href="#">'.$a.'<span class="sr-only">(current)</span></a></li>';
            }
            else
            {
                $cuerpo_pag.='<li><a href="#" onclick="'.$this->nom_fun.'('.$this->parametro.','.(($i)*$this->limit).','.$this->limit.','.$i.')">'.$a.'<span class="sr-only">(current)</span></a></li>';
            }    
        }
        /*if($this->actual< $this->cuantos_enlaces)
        {
