...
else
{
	for($i=0;$i<=count($rs)-1;$i++)
	{
		...
		}else
		{
			...
			{
				$boton_ir_aula="";
				$boton_insc_aula="<button class='btn btn-warning operaciones_be' id='btn_selec_dest".$k."'  onclick='btn_el_inbox(".$rs[$i][0].",".$k.");' title='Inscribirme en esta aula'><i class='fa fa-check'></i></button>";
			}	
		}
		...
		$cuerpo_contenido.="<tr>
								...
								<td width='15%'>".$rs[$i][1]."</td>
								<td width='10%'>".$rs[$i][10]."</td>
								<td width='30%'>".$resumen_aula."</td>
								<td width='10%'>".$rs[$i][6]."</td>
								<td  id='botonera".$k."' width='30%'>".$boton_ir_aula." ".$boton_insc_aula." <button class='btn btn-primary operaciones_be' style='margin-left:2%' title='Consultar aula' id='btn_cons_eva".$k."'onclick='consultar_resumen_eva(".$rs[$i][0].");' ><i class='fa fa-search'></i></button>".$boton_evaluacion."</td>	
					 		</tr>";
		//<td width='5%'>".$rs[$i][2]."</td> 3era linea...											 		
		$imagen_eva='';
	}
	if($actual=="")$actual=0;
